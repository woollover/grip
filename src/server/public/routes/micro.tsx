...
function renderReplies(db: Database, noteId: string): string {
  ...
  return `<section class="replies">${replies.map(r => `
    <article>
      <footer>
        <small>
          <a href="${r.actor_uri}" rel="noopener noreferrer" target="_blank">${r.actor_name}</a>
          &middot; ${new Date(r.published_at).toISOString()}
        </small>
      </footer>
      <div>${r.content}</div>
    </article>`).join('')}
...
export function renderMicroList(db: Database, siteTitle: string, apCfg?: ApConfig | null): string {
  ...
  const content = `
    ...
    ${posts.map(p => `
      ...
      ${showReplies ? renderReplies(db, p.id) : ''}`).join('')}
  `;
