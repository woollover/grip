...
`))
  ...
  function skipSpaceBack(line, i2, to) {
    while (i2 > to && space(line.charCodeAt(i2 - 1)))
      i2--;
    return i2;
  }
...
` ? "" : next;
    ...
    addNode(block, from, to) {
      if (typeof block == "number")
        block = new Tree(this.parser.nodeSet.types[block], none2, none2, (to !== null && to !== undefined ? to : this.prevLineEnd()) - from);
...
`)
  ...
  class Stack {
    ...
    shift(action, type, start, end) {
      if (action & 131072) {
        ...
        this.pos = end;
        let skipped = parser2.stateFlag(nextState, 1);
  ...
  class HistEvent {
    ...
    static fromTransaction(tr, selection) {
      ...
      if (!effects.length && tr.changes.empty)
        return null;
      return new HistEvent(tr.changes.invert(tr.startState.doc), effects, undefined, selection || tr.startState.selection, none3);
    }
    static selection(selections) {
      return new HistEvent(undefined, none3, undefined, undefined, selections);
    }
  }
      ...
      return [{
        ...
        create(view) {
          ...
          btn("Link", "Link (Ctrl+K)", () => {
            ...
            if (!url)
              return;
            const { from: from2, to } = view.state.selection.main;
            const sel = view.state.sliceDoc(from2, to);
            view.dispatch({ changes: { from: from2, to, insert: `[${sel}](${url})` } });
          });
  ...
  var typewriterEffect = StateEffect.define();
  var typewriterState = StateField.define({
    create: () => false,
    update: (v, tr) => {
      for (const e of tr.effects)
        if (e.is(typewriterEffect))
          return e.value;
      return v;
    }
  });
  var typewriterListener = EditorView.updateListener.of((u) => {
    if (!u.state.field(typewriterState))
      return;
  ...
  var focusEffect = StateEffect.define();
  var focusState = StateField.define({
    create: () => false,
    update: (v, tr) => {
      for (const e of tr.effects)
        if (e.is(focusEffect))
  ...
  var dimMark = Decoration.mark({ class: "cm-dim" });
  var focusPlugin = ViewPlugin.fromClass(class {
    decorations = Decoration.none;
    update(u) {
      const toggled = u.transactions.some((t2) => t2.effects.some((e) => e.is(focusEffect)));
      if (!toggled && !u.docChanged && !u.selectionSet)
        return;
      this.decorations = this.build(u.view);
    }
    build(view) {
      if (!view.state.field(focusState))
      ...
      while (end < view.state.doc.length) {
        const nextPos = end + 1;
        if (nextPos > view.state.doc.length)
          break;
      ...
      const ranges = [];
      if (start > 0)
        ranges.push(dimMark.range(0, start));
      if (end < view.state.doc.length)
        ranges.push(dimMark.range(end, view.state.doc.length));
      return ranges.length ? Decoration.set(ranges, true) : Decoration.none;
    }
  }, { decorations: (v) => v.decorations });
  var dragDropHandler = EditorView.domEventHandlers({
    dragover(e) {
    ...
    drop(e, view) {
      ...
      fetch("/media/upload", { method: "POST", body: fd }).then(async (r) => {
        if (!r.ok)
          throw new Error(`${r.status}`);
        const { url, alt } = await r.json();
...
` } });
  ...
  var gripTheme = EditorView.theme({
    "&": { height: "auto" },
  ...
  function init(mountEl, hiddenEl, micro) {
    ...
    window.__gripEditor = view;
    if (!micro) {
      document.getElementById("ws-typewriter")?.addEventListener("click", function() {
      ...
      document.getElementById("ws-focus")?.addEventListener("click", function() {
